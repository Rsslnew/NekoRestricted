...
from config import FORWARD_CHAT_ID, MAX_CONCURRENT_DOWNLOADS, FREE_DAILY_LIMIT
...
def register(bot: Client, db, get_user_client):
    ...
    async def download_media(message, file_path, progress_message=None):
        ...
        user_active_files[user_id].add(file_path)
        ...
        try:
            ...
            user_active_files.get(user_id, set()).discard(file_path)
            return result
        except Exception as e:
            tracker.stop()
            active_downloads.pop(user_id, None)
    ...
    async def cancel_command(client, message):
        ...
        files_to_remove = list(user_active_files.get(user_id, set()))
        removed_count = 0
        for f in files_to_remove:
            try:
                ...
                pass
        user_active_files.pop(user_id, None)
    ...
    async def batch_download(client, message):
        ...
        if total > MAX_BATCH_SIZE:
            return await message.reply_text(
                f"{NEKO_ANGRY}\n\n"
                f"**Batch too large!** 😿\n"
                f"📊 Requested: {total} posts\n"
                f"🔒 Maximum allowed: {MAX_BATCH_SIZE} posts\n\n"
        ...
        if user:
            ...
            if last_reset:
                ...
                if last_reset_str == today:
                    current_daily = user.get("daily_count", 0)

        is_premium = await db.is_premium(user_id)
        if not is_premium and (current_daily + total) > FREE_DAILY_LIMIT:
            remaining = FREE_DAILY_LIMIT - current_daily
            return await message.reply_text(
                f"📊 **Daily Limit Insufficient!**\n\n"
                f"🔴 Current: {current_daily}/{FREE_DAILY_LIMIT}\n"
                f"📦 Batch size: {total}\n"
